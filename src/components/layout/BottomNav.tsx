...
export function BottomNav() {
  ...
  const items = [
    ...
    { href: '/create', label: '', isFab: true },
    { href: '/profile', label: t('nav.profile') },
    { href: '/settings', label: t('nav.settings') },
  ]

  return (
    <nav className="fixed left-0 right-0 z-40 bg-white border-t border-gray-200 lg:hidden bottom-safe">
        ...
        {items.map(item => {
          ...
          if (item.isFab) {
            return (
              <Link
                ...
                className={`flex flex-1 flex-col items-center justify-end gap-0.5 rounded-lg text-xs transition-colors -mt-5 ${active ? 'text-teal-700' : 'text-gray-500'}`}
              >
                <span className="flex items-center justify-center w-12 h-12 rounded-2xl bg-teal-600 text-white shadow-lg hover:bg-teal-700 active:bg-teal-800 active:scale-95 transition-all duration-150">
          ...
          return (
            <Link
              ...
              className={`flex flex-1 min-h-[48px] flex-col items-center justify-center gap-0.5 rounded-lg px-1 py-1 text-xs transition-colors ${active ? 'text-teal-700 font-semibold' : 'text-gray-500 hover:text-teal-600'}`}
            >
              ...
              {item.label && <span className="truncate max-w-full">{item.label}</span>}
            </Link>
          )
        })}
      </div>

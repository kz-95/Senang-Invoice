...
**Onboarding carousel (mobile first-run).** 3 slides (welcome / scope / drive-sync). Big icon tile `rounded-2xl bg-teal-50 text-teal-700 h-20 w-20`; heading `text-3xl font-bold text-teal-900`; progress dots (active `w-6 bg-teal-600`, idle `w-2 bg-gray-300`); **Skip** top-right; full-width primary CTA bottom. Safe-area aware (`env(safe-area-inset-*)`).

**Desktop landing (app-download-first).** Hero: "Download the App Now!" + large APK / Play Store CTAs (`rounded-2xl`, `shadow-cta`), "Malaysian E-Invoice Made Simple", feature pills (`rounded-full bg-white shadow-sm ring-1 ring-gray-100`), phone mockup (invoice + QR + RM totals). Then **PainVsEase** (gray card vs teal card, both `rounded-2xl`), **TrustRow** (3 feature cards), **GuideTeaser** (4 article cards, `rounded-2xl`, hover `hover:border-teal-300 hover:shadow-md`), **CtaFooter** (`bg-teal-700`, white CTA).
...
**Download gates:** Hero APK button, Play Store CTA, and footer "Download Now" all display "Coming Soon" state when `APK_DOWNLOAD_URL` env var is empty or `PLAYSTORE_COMING_SOON` flag is true. User sees disabled button, not broken link.
